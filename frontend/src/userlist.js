...
const ViewModal = ({ selectedUser }) => {
  const getAvatarUrl = useCallback((avatar) => {
    ...
    return `/storage/avatars/${cleanAvatar}?t=${Date.now()}`;
  }, []);
  ...
  const fetchUsers = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const response = await axios.get(`/api/listuser?page=${page}`, config);
  ...
  const handleView = useCallback(
    async (id) => {
      try {
        ...
        const response = await axios.get(`/api/listuser/${id}`, config);
        console.log('Phản hồi:', response.data);
  ...
  const handleEditSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const response = await axios.post(`/api/update/${editForm.id}`, formData, config);
